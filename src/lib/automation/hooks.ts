...
export async function onInquiryCreated(inquiry: any) {
  // Skip if inquiry is not properly formed
  if (!inquiry || !inquiry.id) {
    console.warn('onInquiryCreated called with invalid inquiry object')
    return
  }
  ...
  try {
    ...
    console.error('Error in onInquiryCreated hook:', error)
    // Don't throw - let the inquiry creation succeed even if automation fails
  }
}
...
export async function onInquiryStatusChanged(
  ...
  try {
    await automationEngine.executeRulesForTrigger({
      ...
      userId: userId || inquiry.updatedById || inquiry.createdById
    })
  } catch (error) {
    console.error('Error in onInquiryStatusChanged hook:', error)
  }
}
...
export async function onItemAssigned(
  ...
  try {
    ...
    console.error('Error in onItemAssigned hook:', error)
  }
}
...
export async function onCostCalculated(
  ...
  try {
    await automationEngine.executeRulesForTrigger({
      ...
      userId: userId || calculation.calculatedById
    })
  } catch (error) {
    console.error('Error in onCostCalculated hook:', error)
  }
}
...
export async function onApprovalRequired(
  ...
  try {
    await automationEngine.executeRulesForTrigger({
      ...
      userId: userId || approval.requestedById
    })
  } catch (error) {
    console.error('Error in onApprovalRequired hook:', error)
  }
}

// Hook to be called when quote is created
export async function onQuoteCreated(quoteId: string, quote: any, userId?: string) {
  try {
    await automationEngine.executeRulesForTrigger({
      ruleId: '',
    ...
    console.error('Error in onQuoteCreated hook:', error)
  }
}
...
export async function onProductionOrderCreated(
  ...
  try {
    await automationEngine.executeRulesForTrigger({
      ...
      userId: userId || order.createdById
    })
  } catch (error) {
    console.error('Error in onProductionOrderCreated hook:', error)
  }
}

// Hook to check workload and trigger balancing if needed
export async function checkWorkloadBalance(role: string, userId?: string) {
  try {
    await automationEngine.executeRulesForTrigger({
      ruleId: '',
    ...
    console.error('Error in checkWorkloadBalance hook:', error)
  }
}